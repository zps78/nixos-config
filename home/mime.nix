...
# since those apps (home/niri.nix, home/gnome.nix) have no myApps.*.enable
# of their own - always-on for every niri/gnome host, not something meant
# to vary per user. Both stacks land on the same app set (nautilus/papers/
# loupe/file-roller/foliate), niri.nix and gnome.nix just differ in how
# much else they bring along (Noctalia, KDE-app hiding, etc.).
{ config, lib, osConfig, ... }:

let
  app = config.myApps;
  isGnomeEcosystem = builtins.elem osConfig.myDesktop.stack [ "niri" "gnome" ];
...
in
{
  xdg.mimeApps = {
    ...
    defaultApplications = lib.mkMerge [
      ...
      # ----- GNOME-ecosystem app set (niri + gnome) -----
      # Replaces the old dolphin-era KDE set. papers (GTK4/libadwaita,
      # evince's successor - nixpkgs now ships both, but GNOME upstream
      # has moved on) covers pdf plus every comic-book format okular did,
      # plus djvu - but unlike evince it drops postscript/eps/dvi/xps
      # support entirely (confirmed via its buildInputs: no ghostscript,
      # no libspectre, no texlive-bin, no libgxps), and it drops
      # epub/mobi same as evince did, hence foliate picking those up
      # separately. Verified each app's real MimeType coverage against
      # its own .desktop file rather than assuming.

      (lib.mkIf isGnomeEcosystem (forEach loupe [
        "image/jpeg" "image/png" "image/gif" "image/webp" "image/avif"
        "image/heic" "image/bmp" "image/tiff" "image/svg+xml" "image/jxl"
      ]))

      (lib.mkIf isGnomeEcosystem {
